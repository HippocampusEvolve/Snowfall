...
// становится `tex/snow_02_diff_1k.webp?v=1a2b3c4d`, и это ключ браузерного
...
// как и раньше.
//
// Раньше здесь же сжимались модели: `.glb` и `.bin` отдаются как
// application/octet-stream, и в списки типов для сжатия на лету этот тип не
// входит намеренно. Моделей файлами в сборке не осталось, но правило остаётся
// общим - оно решает по замеру, а не по расширению.
// ---------------------------------------------------------------------------

// Форматы, которые уже сжаты внутри себя: второй проход даёт проценты, а
// место и время занимает целиком. Остальное решает не расширение, а замер ниже.
