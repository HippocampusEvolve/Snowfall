...
export class GameAudio {
  ...
  /**
   * Громкость мира на входе: 0 — тишина, 1 — как задумано.
   *
   * Ведёт её пробуждение (`awaken.js`), чтобы ветер и огонь приходили вместе
   * со светом, а не включались разом на фоне почти трёхсекундного появления.
   * Постоянная времени небольшая: кривую задаёт вызывающий, здесь только
   * сглаживание ступенек, иначе параметр щёлкает.
   */
  setWake(level) {
    // Уровень запоминается ВСЕГДА, даже когда звука ещё нет. Контекст рождается
    // по первому жесту, то есть ровно в момент входа в мир, и без этой памяти
    // возможен порядок, при котором пробуждение успело договорить свои
    // setWake(1) до init() — и мир остался бы немым насовсем.
    this._wake = level;
    if (!this.master) return;
    const ctx = this.master.context;
    this.master.gain.setTargetAtTime(this._baseGain * level, ctx.currentTime, 0.12);
  }
  ...
  init() {
    ...
    // Базовая громкость мира. Хранится отдельно, потому что вход в мир ведёт
    // её множителем (см. setWake выше), и после пробуждения надо вернуться
    // именно к этому значению, а не к тому, что осталось от анимации.
    this._baseGain = 0.85;
    // Начинаем с того уровня, до которого пробуждение уже дошло: обычно это
    // ноль (вход только что нажат), но порядок вызовов гарантий не даёт.
    this.master.gain.value = this._baseGain * (this._wake ?? 0);
