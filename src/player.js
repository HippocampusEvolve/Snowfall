...
export class Player {
  ...
  constructor(camera, look, terrain, onStep, obstacles = [], digger = null, getFloor = null, onLand = null) {
    ...
    this.vy = 0;
    this.grounded = true;
    // Высота, на которой игрока держит сейв, пока не приехала изба (см.
    // «опора, которой ещё нет» в update). null - держать не надо.
    this.holdY = null;
  ...
  /**
   * Остановить тело насовсем: игрок замёрз. На мыши это делает сам браузер,
   * отпуская курсор, а на таче держаться нечему - там `locked` стоит на
   * активности пальцев, и с зажатым пальцем тело продолжало идти за чёрным
   * экраном смерти, а автосейв записывал этот дрейф.
   */
  halt() {
    this.keys.clear();
    this.touch.f = this.touch.r = 0;
   ...
   *
   * Нужен ровно один раз - когда сейв вернул игрока на прежнее место. Тело
   * туда переезжает сразу, а камера ходит за телом только в `update`, которого
   * во время пробуждения нет вовсе: им в это время правит `awaken.js`. Без
   * этой строки вернувшийся весь вход смотрел из точки старта, а на последнем
   * кадре пробуждения его переносило к сейву - это и ощущалось телепортом.
   */
  syncCamera() {
  ...
  update(dt) {
    ...
    // Опора, которой ещё нет. Изба приезжает по сети уже после сборки мира
    // (волна отделки в main.js), а сейв возвращает игрока туда, где он вышел -
    // может быть, и на её пол. Между этими двумя моментами пола под ногами
    // физически нет, и без оговорки вернувшийся встречал бы мир падением с
    // крыльца: удар, звук, клевок камеры, - а через секунду его поднимало бы
    // обратно на доски приехавшим полом.
    //
    // Поэтому пока отделка едет, игрок держится на той высоте, на которой
    // стоял, - как на полу, которого ещё нет. Оговорка снимается сама, как
    // только под ногами окажется настоящая опора не ниже этой (приехал пол,
    // поднялся по склону), и в любом случае снимается волной отделки.
    // Прыжок и ходьба при этом работают как обычно; идущий вниз по склону эту
    // секунду проходит по ровному - в кадре у чужого крыльца это не читается,
    // а падать вернувшемуся на ровном месте нечего.
    if (this.holdY !== null) {
