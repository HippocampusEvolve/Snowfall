...
// Штампуем не только то, что просят через asset(). Загрузчики three ходят за
// своим сами, мимо нашего кода: текстура снега тянется из snowmaterial.js
// через TextureLoader. Всё это идёт через один общий DefaultLoadingManager,
// поэтому достаточно одного перехватчика: он видит каждый адрес, который
// three.js собирается запросить.
//
// Раньше сюда попадали и внутренние ссылки .gltf — на .bin и на карты, — но
// моделей файлами в мире больше нет: всё собирается кодом (src/props/).
//
