// Таблицы Marching Cubes (Paul Bourke, классические 256 случаев).
// edgeTable[cubeIndex] — битовая маска рёбер, пересечённых поверхностью.
// triTable[cubeIndex] — до 5 треугольников (индексы рёбер), терминатор -1.
...
export const triTable = new Int8Array(256 * 16).fill(-1);
for (let i = 0; i < 256; i++) {
  const row = TRI[i];
  for (let j = 0; j < row.length; j++) triTable[i * 16 + j] = row[j];
}
