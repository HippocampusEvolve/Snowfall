...
// зазор макушки от свода (м): на столько капсула не доводится до потолка
const HEAD_MARGIN = 0.08;
// глубже этого макушку из свода не вытаскиваем: за кадр так далеко в грунт не
// уйти, а марш длиннее стоил бы лишних сэмплов поля
const HEAD_PUSH_MAX = 0.5;
const HEAD_STEP = 0.1; // шаг марша под макушкой
...
export function createSupport({ terrain, digger, colliders, getFloor, structure = null }) {
  ...
  return {
    ...
    resolve(pos, radius, height) {
      ...
      // Потолок. Ядро знает только пол: вертикаль оно зажимает опорой снизу и
      // ничем сверху, а горизонтальное выталкивание выше нарочно пропускает
      // почти вертикальный градиент (это пол или свод, а не стена). Значит
      // свод пещеры не держал никого: прыжок в низком ходе уводил макушку в
      // грунт. Опускаем капсулу ровно настолько, чтобы макушка вышла из
      // твёрдого; скорость ядро погасит само, встретив опору.
      if (dig && (dig.caves || dig.edits.size > 0)) {
        const head = pos.y + height + HEAD_MARGIN;
        if (dig.densityAt(pos.x, head, pos.z) > 0) {
          // Опускаем до ближайшего воздуха ПОД макушкой. По градиенту это
          // делать нельзя: в верхних полутора метрах грунта ближайшая
          // поверхность - снег наверху, поле растёт ВНИЗ, и ньютон уводил бы
          // капсулу вверх, сквозь свод. Марш же спрашивает ровно то, что нужно:
          // где кончается твёрдое над головой.
          for (let d = HEAD_STEP; d <= HEAD_PUSH_MAX + 1e-9; d += HEAD_STEP) {
            if (dig.densityAt(pos.x, head - d, pos.z) <= 0) { pos.y -= d; break; }
          }
        }
      }
